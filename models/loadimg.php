...
if(!isset($_POST['imgname'])){
	...
	$num= explode("|", $_POST['imgname'])[1];

	// Директория куда будут загружаться файлы.
	$path = realpath(dirname(__FILE__) . '/..').'/data/product/img/'.explode("|", $_POST['imgname'])[0];
	...
	if (!isset($_FILES["img"])) {
		...
		if (!empty($file['error']) || empty($file['tmp_name'])) {
			$error = 'Занадто великий файл, макс 10мб';
			...
			if (empty($name) || empty($parts['extension'])) {
				...
				if (move_uploaded_file($file['tmp_name'], $path."/".$num.".jpg")) {
